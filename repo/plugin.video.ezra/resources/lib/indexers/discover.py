...
class Discover:
	...
	def movie(self):
		...
		if not 'recommended' in names: self._add_dir({'mode': 'discover.similar_recommended', 'media_type': 'movie', 'key': 'similar', 'list_name': '[B]%s %s:[/B]  [I]%s[/I]' \
																										% (ls(32451), ls(32592), names.get('similar', ''))})
		if not 'similar' in names: self._add_dir({'mode': 'discover.similar_recommended', 'media_type': 'movie', 'key': 'recommended', 'list_name': '[B]%s %s:[/B]  [I]%s[/I]' \
																										% (ls(32451), ls(32593), names.get('recommended', ''))})
		if not any(i in names for i in ('similar', 'recommended')):
			...
			self._add_dir({'mode': 'discover.include_genres', 'media_type': 'movie', 'list_name': self.base_str \
																										% (self.include_base_str % ls(32470), names.get('include_genres', ''))})
			self._add_dir({'mode': 'discover.exclude_genres', 'media_type': 'movie', 'list_name': self.base_str \
																										% (self.exclude_base_str % ls(32470), names.get('exclude_genres', ''))})
			self._add_dir({'mode': 'discover.include_keywords', 'media_type': 'movie', 'list_name': self.base_str % (self.include_base_str \
																										% ls(32657), names.get('include_keywords', ''))})
			self._add_dir({'mode': 'discover.exclude_keywords', 'media_type': 'movie', 'list_name': self.base_str % (self.exclude_base_str \
																										% ls(32657), names.get('exclude_keywords', ''))})
	...
	def tvshow(self):
		...
		if not 'recommended' in names: self._add_dir({'mode': 'discover.similar_recommended', 'media_type': 'tvshow', 'key': 'similar', 'list_name': '[B]%s %s:[/B]  [I]%s[/I]' \
																										% (ls(32451), ls(32592), names.get('similar', ''))})
		if not 'similar' in names: self._add_dir({'mode': 'discover.similar_recommended', 'media_type': 'tvshow', 'key': 'recommended', 'list_name': '[B]%s %s:[/B]  [I]%s[/I]' \
																										% (ls(32451), ls(32593), names.get('recommended', ''))})
		if not any(i in names for i in ['similar', 'recommended']):
			...
			self._add_dir({'mode': 'discover.include_genres', 'media_type': 'tvshow', 'list_name': self.base_str \
																										% (self.include_base_str % ls(32470), names.get('include_genres', ''))})
			self._add_dir({'mode': 'discover.exclude_genres', 'media_type': 'tvshow', 'list_name': self.base_str \
																										% (self.exclude_base_str % ls(32470), names.get('exclude_genres', ''))})
			self._add_dir({'mode': 'discover.include_keywords', 'media_type': 'tvshow', 'list_name': self.base_str \
				                                                                        				% (self.include_base_str % ls(32657), names.get('include_keywords', ''))})
			self._add_dir({'mode': 'discover.exclude_keywords', 'media_type': 'tvshow', 'list_name': self.base_str \
				                                                                       					% (self.exclude_base_str % ls(32657), names.get('exclude_keywords', ''))})
	...
	def cast(self):
		...
		url = 'https://api.themoviedb.org/3/search/person?api_key=%s&language=en-US&query=%s' % (self.tmdb_api, search_name)
	...
	def export(self):
		try:
			...
			final_params = {'name': name, 'mode': mode, 'action': action, 'query': query, 'iconImage': 'discover.png'}
			url_params = {'mode': 'navigator.adjust_main_lists', 'method': 'add_external',
						'list_name': name, 'menu_item': json.dumps(final_params)}
			url_params = {'mode': 'menu_editor.add_external', 'name': name, 'iconImage': 'discover.png'}
		...
		def _builder():
			for count, item in enumerate(data):
				try:
					...
					remove_all_params = {'mode': 'discover.remove_all_history', 'media_type': media_type}
	...
	def _set_default_params(self, media_type):
		if not 'media_type' in self.discover_params:
			...
			self.discover_params['search_string']['base'] = 'https://api.themoviedb.org/3/discover/%s?api_key=%s&language=en-US&page=%s' \
																				% (url_media_type, self.tmdb_api, '%s')
			self.discover_params['search_string']['base_similar'] = 'https://api.themoviedb.org/3/%s/%s/similar?api_key=%s&language=en-US&page=%s' \
																				% (url_media_type, '%s', self.tmdb_api, '%s')
			self.discover_params['search_string']['base_recommended'] = 'https://api.themoviedb.org/3/%s/%s/recommendations?api_key=%s&language=en-US&page=%s' \
																				% (url_media_type, '%s', self.tmdb_api, '%s')
	...
	def _add_defaults(self):
		...
		self._add_dir({'mode': 'discover.export', 'media_type': self.media_type, 'list_name': ls(32667) % name}, icon=kodi_utils.translate_path(icon_directory % 'nextpage.png'))
