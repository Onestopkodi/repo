...
class Images():
		...
		def builder():
			for item in image_info['results']:
				if item['profile_path']:
					...
					actor_poster = os.path.join(icon_directory, 'genre_family.png')
					actor_image = os.path.join(icon_directory, 'genre_family.png')
	...
	def people_image_results(self):
		...
		if page_no == 1:
			...
			tmdb_images = [('%s_%sx%s_%03d' % (actor_name, i['height'], i['width'], count), tmdb_image_base \
									% ('original', i['file_path']), tmdb_image_base % ('w185', i['file_path'])) for count, i in enumerate(tmdb_image_info, rolling_count+1)]
