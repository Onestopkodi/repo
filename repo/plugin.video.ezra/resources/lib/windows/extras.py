...
from modules import watched_status, settings
...
class Extras(BaseDialog):
	...
	def get_progress(self):
		try: resume_point, curr_time, resume_id = watched_status.detect_bookmark(watched_status.get_bookmarks(self.watched_indicators, 'movie'), self.tmdb_id)
		...
		if resume_point in (0, '0', 0.0, '0.0'):
			...
			playcount, overlay = watched_status.get_watched_status_movie(watched_info, str(self.tmdb_id))
	...
	def set_starting_constants(self, kwargs):
		...
		self.clearlogo = self.meta['clearlogo'] if settings.get_fanart_data() else ''
