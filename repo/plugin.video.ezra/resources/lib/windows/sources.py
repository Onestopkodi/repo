...
extra_info_choices = (('PACK', '[B]PACK[/B]'), ('DOLBY VISION', '[B]D/VISION[/B]'), ('HIGH DYNAMIC RANGE (HDR)', '[B]HDR[/B]'), ('HYBRID', '[B]HYBRID[/B]'), 
...
class SourceResults(BaseDialog):
	...
	def set_properties(self):
		...
		self.setProperty('tikiskins.clearlogo', self.meta['clearlogo'] if get_fanart_data() else '')
	...
	def filter_results(self):
		...
		if main_choice in ('quality', 'provider'):
			...
			duplicates = set()
			provider_choices = [i.getProperty(filter_property) for i in self.item_list \
