...
class Downloader:
	...
	def download_check(self):
		...
		if not self.resp: self.return_notification(ok_dialog=32490)
		...
		if self.content < 1: self.return_notification(ok_dialog=32490)
		self.size = 1024 * 1024
	...
	def return_notification(self, notification=None, ok_dialog=None):
		kodi_utils.hide_busy_dialog()
		if notification: kodi_utils.notification(notification, 2500)
