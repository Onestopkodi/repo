...
class CheckSettingsFile:
	def run(self):
		try:
			xbmc.log('[ script.module.ezscrapers ]  CheckSettingsFile Service Starting...', LOGINFO)
			...
			return xbmc.log('[ script.module.ezscrapers ]  Finished CheckSettingsFile Service', LOGINFO)
		except:
			import traceback
			traceback.print_exc()
...
class SettingsMonitor(control.monitor_class):
	def __init__ (self):
		...
		xbmc.log('[ script.module.ezscrapers ]  Settings Monitor Service Starting...', LOGINFO)
...
class AddonCheckUpdate:
	def run(self):
		xbmc.log('[ script.module.ezscrapers ]  Addon checking available updates', LOGINFO)
		try:
			import re
			import requests
			...
			if repo_xml.status_code != 200:
				return xbmc.log('[ script.module.ezscrapers ]  Could not connect to remote repo XML: status code = %s' % repo_xml.status_code, LOGINFO)
			repo_version = re.search(r'<addon id=\"script.module.ezscrapers\".*version=\"(\d*.\d*.\d*)\"', repo_xml.text, re.I).group(1)
			...
			if check_version_numbers(local_version, repo_version):
				while control.condVisibility('Library.IsScanningVideo'):
					control.sleep(10000)
				xbmc.log('[ script.module.ezscrapers ]  A newer version is available. Installed Version: v%s, Repo Version: v%s' % (local_version, repo_version), LOGINFO)
				control.notification(message=control.lang(32037) % repo_version, time=5000)
			return xbmc.log('[ script.module.ezscrapers ]  Addon update check complete', LOGINFO)
		except:
			import traceback
			traceback.print_exc()
...
class SyncMyAccounts:
	def run(self):
		xbmc.log('[ script.module.ezscrapers ]  Sync "My Accounts" Service Starting...', LOGINFO)
		control.syncMyAccounts(silent=True)
		return xbmc.log('[ script.module.ezscrapers ]  Finished Sync "My Accounts" Service', LOGINFO)

class CheckUndesirablesDatabase:
	def run(self):
		xbmc.log('[ script.module.ezscrapers ]  "CheckUndesirablesDatabase" Service Starting...', LOGINFO)
		...
		try:
			...
			import traceback
			traceback.print_exc()
		return xbmc.log('[ script.module.ezscrapers ]  Finished "CheckUndesirablesDatabase" Service', LOGINFO)

def main():
	while not control.monitor.abortRequested():
		xbmc.log('[ script.module.ezscrapers ]  Service Started', LOGINFO)
		...
		if control.isVersionUpdate():
			...
			xbmc.log('[ script.module.ezscrapers ]  Settings file cleaned complete', LOGINFO)
		SettingsMonitor().waitForAbort()
		xbmc.log('[ script.module.ezscrapers ]  Service Stopped', LOGINFO)
		break
