...
class MenuEditor:
	def __init__(self, params):
		self.params = params
		self.menu_item = dict(parse_qsl(kodi_utils.get_infolabel("ListItem.FileNameAndPath").replace('plugin://plugin.video.ezra/?','')))
	...
	def browse(self):
		...
		kodi_utils.execute_builtin('Container.Update(%s)' % kodi_utils.build_url(browse_item))
	...
	def add_original(self):
		...
		if choice == None: return kodi_utils.notification(32736, 1500)
		...
		if position == None: return kodi_utils.notification(32736, 1500)
	...
	def add_trakt(self):
		...
		if trakt_selection == None: return kodi_utils.notification(32736, 1500)
		active_list = self.params_get('active_list')
		list_items = navigator_cache.currently_used_list(active_list)
		...
		if position == None: return kodi_utils.notification(32736, 1500)
		trakt_selection.update({'iconImage': 'trakt.png', 'mode': 'trakt.lists.build_trakt_list'})
	...
	def check_update_list(self):
		...
		if item_position == None: return kodi_utils.notification(32736, 1500)
	...
	def shortcut_folder_add_item(self):
		...
		if len(shortcut_folders) == 1: choice_name, choice_list = shortcut_folders[0]
			...
			choice = self._menu_select([{'name': i[0], 'iconImage': 'genre_foreign.png'} for i in shortcut_folders], '')
			if choice == None: return kodi_utils.notification(32736, 1500)
	...
	def shortcut_folder_add_to_main_menu(self):
		...
		chosen_folder = {'mode': 'navigator.build_shortcut_folder_list', 'name': name, 'iconImage': 'genre_foreign.png', 'shortcut_folder': 'True', 'external_list_item': 'True'}
		active_list = self.params_get('active_list')
		list_items = navigator_cache.currently_used_list(active_list)
